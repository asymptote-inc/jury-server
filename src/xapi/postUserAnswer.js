...
function postUserAnswer(userId, questionId, body, callback) {
  Answer
    .find({ question_id: questionId, userId })
    .exec((errFindAnswer, resAnswer) => {
      if (errFindAnswer) {
        ...
        let answer = new Answer({ userId, question_id: questionId, answer: body.answer });
        let value = 10 /* toxic */
        ...
        answer.save(errSaveAnswer => {
          if (errSaveAnswer) {
            ...
            api.postUserAnswerToQuestion({ questionId, userId, body }, (err, res) => {
              if (err) {
                callback(err);
              } else {
            ...
            User
              .findOneAndUpdate({ userId }, { $inc: { coins: value } })
              .exec((errUser, resUser) => {
                // 
              });

            Question
              .findOneAndUpdate({ question_id: questionId }, { $inc: { answer_count: 1 } })
              .exec((errQuestion, resQuestion) => {
                if (errQuestion || !resQuestion) {
                  // 
                } else {
                  if (resQuestion.answer_count >= resQuestion.answers_per_question) {
                    resQuestion.remove();
                  }
                }
              });
